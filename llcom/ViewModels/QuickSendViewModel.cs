...
public class QuickSendPage
{
    ...
    public QuickSendPage(int index, string name)
    {
        Index = index;
        Name = name;
    }
...
public partial class QuickSendViewModel : ObservableObject
{
    ...
    public ObservableCollection<QuickSendPage> Pages { get; } =
        new ObservableCollection<QuickSendPage>();
    ...
    private void Add()
    {
        Items.Add(
            new ToSendData
            {
                ...
                text = "",
                hex = false,
                commit = Localize("QuickSendButton"),
            }
        );
        Save();
    }
    ...
    private void RemoveAll()
    {
        var (r, s) = Tools.InputDialog.OpenDialog(
            Localize("DeleteConfirmationMsg"),
            "",
            Localize("DeleteConfirmation")
        );
        if (r && s == "YES")
        {
            Items.Clear();
    ...
    private void RenamePage()
    {
        var ret = Tools.InputDialog.OpenDialog(
            "↓↓↓↓↓↓",
            _setting.GetQuickListNameNow(),
            Localize("QuickSendListNameChangeTip")
        );
        if (!ret.Item1)
            return;
    ...
    private void Export()
    {
        ...
        dialog.FileName = System.Text.RegularExpressions.Regex.Replace(
            CurrentPageName,
            "[<>/\\|:\"?*]",
            "-"
        );
